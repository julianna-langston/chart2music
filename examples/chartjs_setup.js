...
const multiAxisCanvas = document.getElementById("multiAxis");
...
window.addEventListener("load", () => {
  ...
  new window.Sonify({
    type: "line",
    title: "AAPL Trades",
    element: multiAxisCanvas,
    cc: document.getElementById("cc-multiAxis"),
    axes: {
      x: {
        label: "Day",
    ...
    data: {
      "Adjusted Close": adjClose.map((y, x) => {
        return {
          ...
          y,
          callback: () => {
            myChart5.setActiveElements([{datasetIndex: 0, index: x}])
            myChart5.update();
          }
        }
      }),
      "Volume": volume.map((y2, x) => {
        return {
          ...
          y2,
          callback: () => {
            myChart5.setActiveElements([{datasetIndex: 1, index: x}])
            myChart5.update();  
          }
        }
      })
