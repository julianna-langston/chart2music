...
const capitalizeFirstLetter = (str) =>
    str.replace(/^\w/, (str) => str.toUpperCase());
...
const rowDict = (table, column, key) => {
    return table
        .map((node) => [node[column], node[key]])
        .reduce((balance, next) => {
            const [region, amount] = next;
            balance[region] ??= 0;
            balance[region] += amount;
            return balance;
        }, {});
};
...
const chartjsToC2mTree = (tree, groups, yKey) => {
    ...
    const recursivelyBuildData = (
        ...
        const thisGroup = myGroups[0];
        const nextGroup = myGroups[1];
        const hasNextLevel = !!nextGroup;

        const dict = rowDict(branch, thisGroup, yKey);
        const row = [];
        const levelEntries = Object.entries(dict).sort((a, b) => {
            if (a[1] > b[1]) {
                return -1;
            }
            if (a[1] < b[1]) {
        ...
        chartjsDataOrder = chartjsDataOrder.concat(
            levelEntries.map(([name]) => name)
        );
        levelEntries.forEach(([group_name, group_value], index) => {
            const nextLevelName = `${nextGroup}_${group_name}`;
            let h = {};
            if (hasNextLevel) {
                const newRow = recursivelyBuildData(
                    branch.filter((node) => node[thisGroup] === group_name),
                    myGroups.slice(1),
                    nextLevelName,
                    level + 1
                );
        ...
        options: {
            ...
            onFocusCallback: ({ point }) => {
                myChart.setActiveElements([
                    { datasetIndex: 0, index: point.custom }
                ]);
                myChart.update();
            }
        }
